...
from typing import Any, Dict, List, Mapping, Optional, Sequence, Type, TypeVar, Literal
...
class ReliabilityPolicy:
    """
    - timeout_s: soft timeout (model/provider)
    - hard_deadline_s: hard cutoff enforced by us (thread future timeout)
...
Provider = Literal["gemini", "openai", "unknown"]
...
class LangChainLLMService(LLMService):
    ...
    Key guarantees:
    - Gemini: uses max_output_tokens (NEVER max_tokens).
    - OpenAI: uses max_tokens.
    - JSON generation: strict schema parsing with repair loop.
    - Hard deadline: enforced with ThreadPoolExecutor.
    """
    ...
    def _invoke_kwargs(self, config: LLMConfig) -> Dict[str, Any]:
        """
        Provider-aware kwargs mapping:
          - Gemini: max_output_tokens only
          - OpenAI: max_tokens
        Also sanitizes config.extra to avoid known provider breakage.
        """
        kwargs: Dict[str, Any] = {}
        ...
        # Provider sanitization: Gemini rejects max_tokens entirely
        if self._provider == "gemini":
    ...
    def _call_with_deadline(
        *,
        ...
        # Uses the service-owned executor so we don't spawn threads per call.
        future = None
        try:
            # model.invoke is blocking, run it in thread
            # NOTE: we pass messages as first arg, kwargs as **kwargs
            executor = getattr(model, "_executor", None)  # ignore; don't depend
            # Use local future creation (service-owned executor stored on self)
            # (This function is static; caller handles executor)
            raise RuntimeError("Use instance _call_with_deadline_instance")
        except Exception:
            # never used directly
            raise

    def _call_with_deadline_instance(
        self,
        *,
        messages: List[BaseMessage],
        kwargs: Dict[str, Any],
        ...
        future = self._executor.submit(self._model.invoke, messages, **kwargs)
        try:
    ...
    def _invoke_once(self, *, messages: List[BaseMessage], config: LLMConfig) -> Any:  # type: ignore[override]
        kwargs = self._invoke_kwargs(config)
        if self._rel.hard_deadline_s is None:
            return self._model.invoke(messages, **kwargs)
        return self._call_with_deadline_instance(messages=messages, kwargs=kwargs, deadline_s=self._rel.hard_deadline_s)

    @staticmethod
    def _sleep_backoff(attempt: int, base: float = 1.5) -> None:
        sleep_s = base * (2**attempt) * (0.8 + 0.4 * random.random())
        time.sleep(sleep_s)

    def _sleep_backoff(self, attempt: int) -> None:
        self._sleep_backoff.__func__(attempt, base=self._rel.base_backoff_s)  # type: ignore[attr-defined]
    ...
    def _is_retriable(self, e: Exception) -> bool:
        # Keep simple: timeouts + common transient network/provider errors
        ...
        if isinstance(content, list):
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    txt = item.get("text")
                    parts.append(txt if isinstance(txt, str) else json.dumps(item, ensure_ascii=False, default=str))
                else:
    ...
    @staticmethod
    def _detect_provider(model: BaseChatModel) -> Provider:
        mod = model.__class__.__module__ or ""
        if "langchain_google_genai" in mod:
            return "gemini"
