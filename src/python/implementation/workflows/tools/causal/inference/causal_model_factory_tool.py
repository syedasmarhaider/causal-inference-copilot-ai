...
class CausalModelFactoryTool(Tool):
    ...
    @classmethod
    def create_default(
        cls, *, data_repo: DataRepo, models_repo: ModelsRepo
    ) -> CausalModelFactoryTool:
        _ = data_repo
        encoding_util = EncodingUtil()
        return cls(
            _by_fqcn={
                "econml.dml.LinearDML": LinearDMLCausalModel(
                    models_repo=models_repo, encoding_util=encoding_util
                ),
                "econml.dml.SparseLinearDML": SparseLinearDMLCausalModel(
                    models_repo=models_repo, encoding_util=encoding_util
                ),
                "econml.dml.KernelDML": KernelDMLCausalModel(
                    models_repo=models_repo, encoding_util=encoding_util
                ),
                "econml.dml.CausalForestDML": CausalForestDMLCausalModel(
                    models_repo=models_repo, encoding_util=encoding_util
                ),
                "econml.dr.LinearDRLearner": LinearDRLearnerCausalModel(
                    models_repo=models_repo, encoding_util=encoding_util
                ),
                "econml.dr.SparseLinearDRLearner": SparseLinearDRLearnerCausalModel(
                    models_repo=models_repo, encoding_util=encoding_util
                ),
                "econml.dr.ForestDRLearner": ForestDRLearnerCausalModel(
                    models_repo=models_repo, encoding_util=encoding_util
                ),
            }
        )
