...
@dataclass(frozen=True, slots=True)
class _BaseRunDML:
    """Normal FIT, ATE, and CATE execution for one configured DML estimator."""

    models_repo: ModelsRepo
    encoding_util: EncodingUtil
    estimator_cls: Any
    backend_name: str
    fit_include_names: set[str]
    use_pre_x_as_featurizer: bool = True
    require_numeric_x: bool = False
    drop_first_effect_modifier_onehot: bool = False
    cate_query_as_numpy: bool = False

    @property
    def ESTIMATOR_CLS(self) -> Any:
    ...
    @property
    def FIT_INCLUDE_NAMES(self) -> set[str]:
        return self.fit_include_names

    @property
    def USE_PRE_X_AS_FEATURIZER(self) -> bool:
        return self.use_pre_x_as_featurizer

    @property
    def REQUIRE_NUMERIC_X(self) -> bool:
        return self.require_numeric_x

    @property
    def DROP_FIRST_EFFECT_MODIFIER_ONEHOT(self) -> bool:
        return self.drop_first_effect_modifier_onehot
