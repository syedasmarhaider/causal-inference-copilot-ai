...
class _CATEOnCombinedFeatures:
    """Expose a fitted DR estimator as DRTester's CATE object."""
...
@dataclass(frozen=True, slots=True)
class _BaseValidateDR:
    """Outer-CV held-out CATE and DR validation for one ``_BaseRunDR``."""
        ...
        *,
        ...
        try:
            ...
            log.info(
                ...
                backend=self.run_dr.BACKEND_NAME,
                outer_cv_folds=n_splits,
                outer_cv_n_jobs=n_jobs,
                inner_dr_cv=_INNER_DR_CV,
            ...
            return CommandFailure(
                ...
                error=ErrorInfo(
                    ...
                    message=f"{self.run_dr.BACKEND_NAME} outer-CV validation failed.",
        ...
        *,
        ...
        with Manager() as manager:
            ...
            log_thread = Thread(
                ...
                args=(progress_queue, stop_event, self.run_dr.BACKEND_NAME),
                daemon=True,
            )
            log_thread.start()
            try:
        ...
        *,
        ...
        try:
            ...
            if not isinstance(cate_result, CATESuccess):
                raise _ValidationFoldError(_failure_message("DR CATE", cate_result))
            cate, cate_lower, cate_upper = _extract_cate(cate_result, len(test_df))

            record = self.run_dr.models_repo.load_model(
                user_id=user_id,
                conversation_id=conversation_id,
                model_id=model_id,
            )
            ...
            if model_id is not None:
                self.run_dr.models_repo.delete_model(
                    user_id=user_id,
                    conversation_id=conversation_id,
                    model_id=model_id,
                )
    ...
    def _run_drtester(
        ...
        *,
        ...
        plan = self.run_dr.encoding_util.compile(
...
def _extract_cate(
    ...
    if len(cate) != expected_rows:
        raise _ValidationFoldError("DR CATE returned the wrong held-out row count.")
    ...
    if len(lower_array) != expected_rows or len(upper_array) != expected_rows:
        raise _ValidationFoldError("DR CATE returned an invalid held-out confidence interval.")
