...
class ConfirmTransformedProtocolState(State):
    ...
    def message(self) -> str:
        if self.payload.user_message is None:
            raise ValueError("ConfirmTransformedProtocolState message is required but missing. State must have user message. Dont call this property if this is not runned in the node context where user_message is guaranteed to be set.")
