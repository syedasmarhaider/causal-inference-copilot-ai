...
class TransformProtocolDeps(StateDep):
    ...
    def from_loaded(cls, loaded: Mapping[str, State]) -> "TransformProtocolDeps":
        ...
        if not isinstance(pd, CompileProtocolState):
            raise ValueError(
                f"TransformProtocolDeps: missing/invalid {CompileProtocolState.NAME} (got {type(pd).__name__ if pd else None})"
            )
        ci = loaded.get(CompileInferenceState.NAME)
        if not isinstance(ci, CompileInferenceState):
            raise ValueError(
                f"TransformProtocolDeps: missing/invalid {CompileInferenceState.NAME} (got {type(ci).__name__ if ci else None})"
            )
