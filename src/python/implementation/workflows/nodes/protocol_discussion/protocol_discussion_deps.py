...
class ProtocolDiscussionDeps:
    ...
    def from_loaded(cls, loaded: Mapping[str, State]) -> ProtocolDiscussionDeps:
        ...
        if ds is None:
            raise StateDependencyError(f"ProtocolDiscussionDeps: missing {DatasetState.NAME}", to_state="ProtocolDiscussionDeps", missing_dependencies=[DatasetState.NAME])
        if not isinstance(ds, DatasetState):
            raise StateDependencyError(f"ProtocolDiscussionDeps: invalid {DatasetState.NAME} "
                                       f"(expected DatasetState, got {type(ds).__name__})", to_state="ProtocolDiscussionDeps", missing_dependencies=[DatasetState.NAME])
