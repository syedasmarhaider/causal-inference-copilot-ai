...
def make_validate_protocol_static_node(*, data_repo: DataRepo, llm: LLMService, model_name: str) -> CallableNodeFunc:
    def _run(user_id: UUID, conversation_id: UUID, state: ConversationState) -> ConversationState:
        ...
        # Apply exclusions (single job: filtering)
...
def _validate_protocol_enums(protocol: ProtocolState) -> List[ProtocolValidationIssue]:
    ...
    # NOTE: You asked: conceptual time zero -> ignore time logic.
...
def _safe_exclusions(protocol: ProtocolState) -> List[ExclusionRule]:
    ex = protocol.get("exclusions")
    return  ex


def _apply_exclusions(df: pd.DataFrame, exclusions: Sequence[ExclusionRule]) -> Tuple[pd.DataFrame, List[ProtocolValidationIssue], Dict[str, Any]]:
    out_df = df
    issues: List[ProtocolValidationIssue] = []
...
def _validate_nonempty_after_exclusions(*, df_before: pd.DataFrame, df_after: pd.DataFrame, excl_metrics: Dict[str, Any]) -> List[ProtocolValidationIssue]:
...
def _build_metrics(*, df_before: pd.DataFrame, df_after: pd.DataFrame, protocol: ProtocolState, excl_metrics: Dict[str, Any]) -> Dict[str, Any]:
    m: Dict[str, Any] = {
...
def _llm_render_validation_message(*, llm: LLMService, model_name: str, report: ProtocolValidationReport) -> str:
    ...
    cfg = LLMConfig(model=model_name, temperature=0.2)

    try:
        resp = llm.generate(
            config=cfg,
            system_prompt="Be concise, actionable, and faithful to the report. No JSON.",
            user_prompt=prompt,
            history=None,
        )
